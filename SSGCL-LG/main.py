...
def main(args):
    ...
    for cur_repeat in range(args['repeat']):
        ...
        model = SSGCLLG(num_meta_paths=len(G),
                     ...
                     num_layerser=args['num_layerser'],
                     num_layersbi=args['num_layersbi'],
        ...
        model.eval()
        logits,_,loss_self, h = model(G, features,labeladj,feat,mate, pos)
        svm_macro, svm_micro = evaluate_results_nc(h[test_idx].detach().cpu().numpy(), labels[test_idx].cpu().numpy(),int(labels.max()) + 1)  # 使用SVM评估节点
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_layerser', default=2, help='网络层数')#1
    parser.add_argument('--num_layersbi', default=1, help='网络层数')#1
